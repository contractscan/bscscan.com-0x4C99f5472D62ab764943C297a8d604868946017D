...
// File: contracts/VINEDividendPayingToken.sol
    ...
    IDividendPayingTokenOptional
{
    ...
    function updateDividendTokenUser(address sender, address _myDividendToken)
        public
        onlyMaster
    {
    ...
    function getDividendTokenUser(address sender)
        public
        view
        returns (address)
    {
...
contract VINE is ERC20, Ownable {
    ...
    VINEDividendTracker public dividendTracker;
    ...
    uint256 public totalFees =
        tokenRewardsFee.add(liquidityFee).add(marketFee).add(buyBackFee).add(
            burnFee
        );
    // buy fees
    ...
    uint256 public buyTokenRewardsFee = 10;
    ...
    function getDividendTokenUser(address _myAddress)
        external
        view
        returns (address)
    {
    ...
    function updateMinimumBalanceForDividends(uint256 newAmountNoDecimals)
        external
        onlyOwner
    {
        dividendTracker.updateMinimumBalanceForDividends(newAmountNoDecimals);
    }

    function updateMaxWalletAmount(uint256 newAmountNoDecimals)
        external
        onlyOwner
    {
        _maxWalletToken = newAmountNoDecimals * (10**18);
    }

    function updateSwapAtAmount(uint256 newAmountNoDecimals)
        external
        onlyOwner
    {
        swapTokensAtAmount = newAmountNoDecimals * (10**18);
    }

    function updateMarketAddress(address payable newAddress)
        external
        onlyOwner
    {
    ...
    function updateBuyBackAddress(address payable newAddress)
        external
        onlyOwner
    {
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(
            ...
            "VINE: The dividend tracker already has that address"
        );

        VINEDividendTracker newDividendTracker = VINEDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "VINE: The new dividend tracker must be owned by the VINE token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(
            ...
            "VINE: The router already has that address"
        );
        uniswapV2Router = IUniswapV2Router02(newAddress);
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "VINE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "VINE: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "VINE: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "VINE: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            require(
                ...
                "VINE: This account cannot send tokens until trading is enabled"
            );
        }
        ...
        if (takeFee) {
            uint256 fees = amount.mul(totalFees - burnFee).div(100);
            uint256 burn = amount.mul(burnFee).div(100);

            amount = amount.sub(fees + burn);
    ...
    function swapAndLiquify(uint256 tokens) internal {
        ...
        uint256 burnFeeBalance = newBalance.div(totalFees).mul(burnFee);
        uint256 burnFeePortion = otherHalf.div(totalFees).mul(burnFee);
        ...
        uint256 walletTotalBalance = marketFeeBalance +
            burnFeeBalance +
            buyBackFeeBalance;
        uint256 walletTotalPortion = marketFeePortion +
            burnFeePortion +
            buyBackFeePortion;
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (DividendToken != address(0)) {
            ...
            success = IERC20(DividendToken).transfer(
...
contract VINEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("VINE_Dividend_Tracker", "VINE_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function updateMinimumBalanceForDividends(uint256 newAmountNoDecimals)
        ...
        onlyOwner
    {
        minimumTokenBalanceForDividends = newAmountNoDecimals * (10**18);
    }
    ...
    function _transfer(
        ...
        require(false, "VINE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "VINE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main VINE contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "VINE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "VINE_Dividend_Tracker: Cannot update claimWait to same value"
        );
